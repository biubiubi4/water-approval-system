...
## 7. 智能审核与完整性检测（变更要点）
为支持智能审核服务，需要在系统设计中加入以下要点：

1. 完整性检测工具：增加 `check_completeness` 工具（位于 Python AI 服务 `app/tools.py`），其输入为 `application` 对象与可选 `materials` 列表，输出为结构化 JSON，包括：
   - `complete` (bool)
   - `applicant_type` ("PERSONAL"|"ENTERPRISE")
   - `submitted_materials` (list)
   - `missing_items` (list)
   - `issues` (list)

2. 前端适配：`CreateApplication` 表单需提供 `applicantType` 字段（个人/企业），并允许附件为可选上传（不再在前端强制三项同时上传），但应提示建议材料。

3. 后端对接：Java 后端在接收前端申请时应保留并传递 `applicantType` 字段到 AI 服务，AI 服务应基于该字段决定是否检查身份证或营业执照。

4. 接口与错误处理：`/api/check-completeness` 与 `/api/review` 接口应返回详细的 `details` 结构，供前端用于展示“材料不完整”或“合规性问题”的标签和建议。

5. 安全与隐私：证件影像等权限敏感文件在存储与传输时需确保加密与访问控制，后端保存路径应限制访问并清理临时文件。

以上为设计层面的变更要点，建议在实现前在 `design` 分支中产出详细接口规范与 JSON 模式样例。 
